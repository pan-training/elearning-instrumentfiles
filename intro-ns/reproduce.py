...
class from_reference:
    def __init__(self, instrument_path, log_path, input_path=".",
                 data_folder="data_folder", debug_folder="debug", mcrun_path=None,
    ...
    def read_instrument(self):
        ...
        if self.could_write_python_file_p.value == 1:
            ...
            string =  "import os\n"
            string += "import sys\n"
            string += "sys.path.append('" + os.path.dirname(os.path.abspath(__file__)) + "')\n"
    ...
    def write_read_instrument_file(self):
        ...
        string += "InstrReader.write_python_file(\"" + self.name + "_file_debug.py\", force=True)\n"
    ...
    def run_reference(self):
        ...
        mcrun_path = self.mcrun_path

        command = mcrun_path + "mcrun " + option_string + " " + self.instrument_path

        # Extract a single parameter
        parameter_list = list(self.instrument_object.parameters)
        if len(parameter_list) > 0:
            first_par = parameter_list[0]
            name = first_par.name
            value = first_par.value
    ...
    def run_file_version(self):
        ...
        try:
            ...
            print("Above command failed with following exception:")
            print(e)
