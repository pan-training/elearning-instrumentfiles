...
def main(argv):
    ...
    # other mcrun options? Can i pipe them?

    try:
        opts, args = getopt.getopt(argv, "hf:i:p:t:n:s:",["folder=", "input=", "mcrun_path=", "tests=", "ncount=", "seed=", "mpi=", "different", "timeout="])
    except getopt.GetoptError:
        print("Error in input, use -h for more information.")
        print("reproduce_examples.py -f <foldername> -i <inputfoldername> -p <mcrun_path> -t <n tests> -n <ncount> -s <seed> --mpi <n cores> --different")
        sys.exit(2)
    ...
    for opt, arg in opts:
        if opt == "-h":
            ...
            print("-i --input : The folder from which insturments are executed, default input_folder")
            print("-p --mcrun_path : path of folder containing mcrun to be used for reference runs")
    ...
    for instrument_path in instrument_paths:
        print("Progress: ", len(reps), "of", N_instruments)
        rep = reproduce.from_reference(instrument_path, log_path, input_path=input_folder,
                                       ...
                                       mcrun_path=mcrun_path, timeout=timeout)
        rep.run_all()
        reps.append(rep)

    print("-"*80)
    for rep in reps:
        rep.print_result_short()
