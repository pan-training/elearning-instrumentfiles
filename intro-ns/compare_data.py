...
class compare_engine:
    ...
    def run_comparison(self, data, data_ref):
        ...
        for data_set in data_ref:
            ...
            # find the data
            name = data_set.metadata.filename
            py_data_set = functions.name_search(name, data)
    ...
    def compare_1d(self, py_data_set, data_set):
        ...
        if not hasattr(py_data_set, "Intensity"):
            print(py_data_set)
            raise RuntimeError("Given py_data_set didn't have Intensity data")
            
        if not hasattr(data_set, "Intensity"):
            print(py_data_set)
            raise RuntimeError("Given data_set didn't have Intensity data")
